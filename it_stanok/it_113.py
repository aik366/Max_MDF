...
from it_stanok import it_112


def it_113(k, dlina, visota, RAM):
    if k[7] in ('СТЕКЛО', 'Хлеб_ст', 'ПЕРЕПЛЕТ', 'ПЕРЕПЛЕТ2', 'ПЕРЕПЛЕТ4', 'ПЕРЕПЛЕТ4_xl', 'ПЕРЕПЛЕТ8'):
        return it_112(k, dlina, visota, RAM)

    txt_ss = ''
    konec, shag, gl = 0, 38, 12.501
    oBv = [30, 29, 12.2, 23.301, 21.401, 25.301, 13, 33.6, 22, 6]

    if k[7] in ('Фасад', 'Хлеб'):
        txt_ss = open('rs/113_it/f.txt', 'r').read()
        ...
        txt_ss = open('rs/113_it/f2.txt', 'r').read()
        RAMX, RAMY, lxp, zaz = RAM[0], RAM[0], RAM[2], 53 + (RAM[0] - 44.5)
        ...
        xdf1_dlina = (RAM[2] - (2 * (RAM[0] + oBv[8] - 2) + 1))
        xdf2_dlina = ((int(dlina) - RAM[2] + zaz) - (2 * (RAM[0] + oBv[8] - 2) + 1))
        xdf_visota = (int(visota) - (2 * (RAM[0] + oBv[8] - 2) + 1))
        ...
        txt_ss = open('rs/113_it/ps.txt', 'r').read()
    ...
    if k[9] == 'БПл':
        txt_ss = txt_ss.replace("12.501, 0, 0, 0, 0, 0", "9.5, 0, 0, 0, 0, 0,")
