from it_stanok import it_106


def it_108(k, dlina, visota, RAM):
    if k[7] in ('ПЕРЕПЛЕТ', 'ПЕРЕПЛЕТ2', 'ПЕРЕПЛЕТ4', 'ПЕРЕПЛЕТ4_xl', 'ПЕРЕПЛЕТ8'):
        return it_106(k, dlina, visota, RAM)

    txt_ss = ''
    konec, shag, gl = 0, 38, 10.001
    D = 26
    oBv = [D, 29, 15.0001, D - 4.601, D - 5.601, 0, D - 6.501, 22, D + 0.5, 13.1]
    oBv[9] = oBv[2] - (oBv[3] - oBv[4])

    if k[7] in ('Фасад', 'Хлеб'):
        txt_ss = open('rs/108_it/f.txt', 'r').read()
        ...
        txt_ss = open('rs/108_it/ps.txt', 'r').read()
