...
def it_56(k, dlina, visota, RAM):
    if k[7] == 'ПСЯ':
        return it_63.it_63(k, dlina, visota, RAM)

    txt_ss = ''
    oBv = [27, 33, 61.5, 14.101, 23.301, 12.201, 21.401, 12, 27.5, 6, 10.4, 1.4]
    ...
    if k[7] == 'Фасад':
        txt_ss = open('rs/56_it/f.txt', 'r').read()
        ...
        oBv[1] = oBv[8]
        txt_ss = open('rs/56_it/st.txt', 'r').read()
        RAMX, RAMY, rRr1, tx0 = 113, 44.5, 21.5, 36.3
        RAMK = RAMY + (int(visota) - 305) * 0.125 if int(visota) > 305 else RAMY + 0.001
        ty0 = (rRr1 ** 2 - (tx0 - rRr1) ** 2) ** 0.5
        tx1 = RAMX - RAMY - tx0
    ...
    elif k[7] == 'Хлеб' and int(visota) < 345:
        txt_ss = open('rs/56_it/xlsl.txt', 'r').read()
        RAMX, RAMY, rRr1, tx0 = 113, 44.5, 21.5, 36.3
        ...
        if int(visota) < 295:
            txt_ss = txt_ss.replace('@ ROUT, 0 : "136_1"', "'" + ' ROUT, 0 : "136_1"')

    elif k[7] == 'Хлеб' and int(visota) >= 345:
        txt_ss = open('rs/56_it/xl.txt', 'r').read()
        RAMX, RAMY, rRr1, tx0 = 113, 44.5, 21.5, 36.3
        RAMK = RAMY + (int(dlina) - 305) * 0.125 if int(dlina) > 305 else RAMY + 0.001
        ty0 = (rRr1 ** 2 - (tx0 - rRr1) ** 2) ** 0.5
        tx1 = RAMX - RAMY - tx0
        ...
        oBv[1] = oBv[8]
        txt_ss = open('rs/56_it/xl_st.txt', 'r').read()
        RAMX, RAMY, rRr1, tx0 = 113, 44.5, 21.5, 36.3
        RAMK = RAMY + (int(dlina) - 305) * 0.125 if int(dlina) > 305 else RAMY + 0.001
        ty0 = (rRr1 ** 2 - (tx0 - rRr1) ** 2) ** 0.5
        tx1 = RAMX - RAMY - tx0
        ...
        txt_ss = open('rs/56_it/but.txt', 'r').read()
        ...
        txt_ss = open('rs/56_it/butsl.txt', 'r').read()
        ...
        txt_ss = open('rs/56_it/butsl2.txt', 'r').read()
        RAMX, RAMY, RAMZ, rRr1, tx0, oBv[0] = 113, 29.5, 44.5, 21.5, 36.3, 17
        RAMK = RAMY + (int(visota) - 305) * 0.125 if int(visota) > 305 else RAMY + 0.001
        ty0 = (rRr1 ** 2 - (tx0 - rRr1) ** 2) ** 0.5
        tx1 = RAMX - RAMZ - tx0
