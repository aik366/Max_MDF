from it_stanok import it_70


def it_71(k, dlina, visota, RAM):
    if k[7] in ('СТЕКЛО', 'Хлеб_ст', 'ПСЯ', 'Бутыл.'):
        return it_70(k, dlina, visota, RAM)

    txt_ss = ''
    oBv = [27, 33, 61.5, 23.301, 21.401, 14.101, 12.201, 12, 27.5, 6, 10.4, 1.4]

    if k[7] == 'Фасад':
        txt_ss = open('rs/71_it/f.txt', 'r').read()
        RAMX, RAMY = 44.5, 44.5
        ...
        txt_ss = open('rs/71_it/xl.txt', 'r').read()
        RAMX, RAMY = 44.5, 44.5
        tx1, tx2, tx3, tx4, tx6 = 2.5, 5, 7.5, 10, 6
