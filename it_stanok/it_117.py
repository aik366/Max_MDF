from it_stanok.it_118 import it_118


def it_117(k, dlina, visota, RAM):
    if k[7] in ('СТЕКЛО', 'Хлеб_ст', 'ПЕРЕПЛЕТ', 'ПЕРЕПЛЕТ2', 'ПЕРЕПЛЕТ4', 'ПЕРЕПЛЕТ4_xl', 'ПЕРЕПЛЕТ8'):
        return it_118(k, dlina, visota, RAM)

    txt_ss = ''

    konec, shag, gl = 0, 38, 10.001
    D = 26
    oBv = [D, 29, D - 22.5, 15, D - 5.601, 0, D - 6.501, 0, 22, -2.9]

    if k[7] in ('Фасад', 'Хлеб'):
        txt_ss = open('rs/117_it/f.txt', 'r').read()
        ...
        txt_ss = open('rs/117_it/ps.txt', 'r').read()
