...
from it_stanok import it_99


def it_103(k, dlina, visota, RAM):
    if k[7] in ('СТЕКЛО', 'Хлеб_ст', 'ПЕРЕПЛЕТ', 'ПЕРЕПЛЕТ2', 'ПЕРЕПЛЕТ4', 'ПЕРЕПЛЕТ4_xl', 'ПЕРЕПЛЕТ8'):
        return it_99(k, dlina, visota, RAM)

    txt_ss = ''
    konec, shag, gl = 0, 38, 12
    D = 26
    oBv = [D, 29, D - 5.1, D - 4.601, D - 5.601, D - 12.901, D - 14.801, D - 15, D + 0.5, 22]

    if k[7] in ('Фасад', 'Хлеб'):
        txt_ss = open('rs/103_it/f.txt', 'r').read()
        ...
        txt_ss = open('rs/103_it/f2.txt', 'r').read()
        RAMX, RAMY, lxp, zaz = RAM[0], RAM[0], RAM[2], 48 + (RAM[0] - 44.5)
        ...
        txt_ss = open('rs/103_it/ps.txt', 'r').read()
    ...
    if k[9] == 'БПл':
        txt_ss = txt_ss.replace("12, 0, 0, 0, 0, 0", "9, 0, 0, 0, 0, 0,")
    return txt_ss
