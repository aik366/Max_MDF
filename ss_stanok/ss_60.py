from ss_stanok import ss_59


def ss_60(k, dlina, visota, RAM):
    if k[7] == 'ПСЯ':
        return ss_59(k, dlina, visota, RAM)

    txt_ss = ''
    oBv = [27, 33, 61.5, 13.6, 22.21, 12.2001, 20.7501, 27.5, 12, 6]
    Kant = '10.4' if k[5] == '18' else '14.8'

    if k[7] == 'Фасад':
        txt_ss = open('rs/60/f.txt', 'r').read()
        RAMX, RAMY = 96, 44.5
        tx1 = RAMX - RAMY
        ...
        txt_ss = open('rs/60/xl.txt', 'r').read()
        RAMX, RAMY = 96, 44.5
        tx1 = RAMX - RAMY
        ...
        txt_ss = open('rs/60/st.txt', 'r').read()
        oBv[1] = oBv[7]
        RAMX, RAMY = 96, 44.5
        tx1 = RAMX - RAMY
        ty1 = (int(visota) - RAMY * 2) / 2
        ...
        txt_ss = open('rs/60/xlst.txt', 'r').read()
        oBv[1] = oBv[7]
        RAMX, RAMY = 96, 44.5
        tx1 = RAMX - RAMY
        ty1 = (int(dlina) - RAMY * 2) / 2
        ...
        txt_ss = open('rs/60/but.txt', 'r').read()
        ...
        txt_ss = open('rs/60/butsl.txt', 'r').read()
        RAMY = 22.5
        RAMX, RAMK, oBv[2] = 80, 35.5, 20
        RAMZ = RAMX - RAMK
        ty0 = (int(visota) - 2 * RAMY) / 2
