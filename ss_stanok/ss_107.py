from ss_stanok import ss_92

def ss_107(k, dlina, visota, RAM):

    if k[7] in ('СТЕКЛО', 'Хлеб_ст', 'ПЕРЕПЛЕТ', 'ПЕРЕПЛЕТ2', 'ПЕРЕПЛЕТ4', 'ПЕРЕПЛЕТ4_xl', 'ПЕРЕПЛЕТ8'):
        return ss_92(k, dlina, visota, RAM)

    txt_ss = ''
    konec, shag, gl = 0, 38.0, 10.5

    oBv = [27, 29, 61.5, 13.6, 22.21, 12.2001, 20.7501, 27.5, 22, 19]

    if k[7] == 'Фасад':
        txt_ss = open('rs/107/f.txt', 'r').read()
        ...
        txt_ss = open('rs/107/xl.txt', 'r').read()
        RAMX, RAMY = RAM[0], RAM[0]
        nachy, nachx = RAMY + oBv[1] + 15, RAMX + oBv[1] + 15
        ...
        txt_ss = open('rs/107/st.txt', 'r').read()
        oBv[1] = oBv[7]
        RAMX, RAMY = RAM[0], RAM[0]
        ...
        txt_ss = open('rs/107/xlst.txt', 'r').read()
        oBv[1] = oBv[7]
        RAMX, RAMY = RAM[0], RAM[0]
        ...
        txt_ss = open('rs/107/ps.txt', 'r').read()
        RAMX, RAMY = RAM[0], RAM[1]
        ...
        txt_ss = open('rs/107/but.txt', 'r').read()
        RAMX, RAMY = RAM[0], RAM[1]
        nachy, nachx, shag = RAMY + oBv[1] + 10, RAMX + oBv[1] + 10, 15
    ...
    if k[9] == "БПл":
        txt_ss = txt_ss.replace('Z=t-10.001', 'Z=t-7').replace('ZA="t-10.001"', 'ZA="t-7"')
        txt_ss = txt_ss.replace('Z=t-10.5', 'Z=t-7.5').replace('ZA="t-10.5"', 'ZA="t-7.5"')
