from ss_stanok.ss_118 import ss_118

def ss_117(k, dlina, visota, RAM):
    if k[7] in ('СТЕКЛО', 'Хлеб_ст', 'ПЕРЕПЛЕТ', 'ПЕРЕПЛЕТ2', 'ПЕРЕПЛЕТ4', 'ПЕРЕПЛЕТ4_xl', 'ПЕРЕПЛЕТ8'):
        return ss_118(k, dlina, visota, RAM)

    txt_ss = ''
    konec, shag, gl = 0, 38.0, 10.001

    oBv = [27, 29, 5.4, 22.3001, 21.2501, 32.2001, 21.2501, 34.0701, 22, 15]

    if k[7] in ('Фасад', 'Хлеб'):
        txt_ss = open('rs/117/f.txt', 'r').read()
        ...
        txt_ss = open('rs/117/ps.txt', 'r').read()
