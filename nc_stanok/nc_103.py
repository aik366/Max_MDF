from nc_stanok import nc_99


def nc_103(k, dlina, visota, RAM):
    if k[7] in ('СТЕКЛО', 'Хлеб_ст', 'ПЕРЕПЛЕТ', 'ПЕРЕПЛЕТ2', 'ПЕРЕПЛЕТ4', 'ПЕРЕПЛЕТ4_xl', 'ПЕРЕПЛЕТ8'):
        return nc_99(k, dlina, visota, RAM)

    txt_ss = ''
    konec, shag, gl= 0, 38.0, 12

    oBv = [27, 29, 61.5, 13.6001, 22.3001, 11.7301, 20.4051, 27.5, 22, 19]

    if k[7] in ('Фасад', 'Хлеб'):
        txt_ss = open('rs/103/f.txt', 'r').read()
        ...
        txt_ss = open('rs/103/ps.txt', 'r').read()
