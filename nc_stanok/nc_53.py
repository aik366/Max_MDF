...
def nc_53(k, dlina, visota, ram):
    if k[7] == 'ПСЯ':
        return nc_63.nc_63(k, dlina, visota, RAM)

    txt_ss = ''
    oBv = [27, 33, 61.5, 13.6001, 22.3001, 11.7301, 20.4301, 12, 27.5, 6, 0, 0]
    oBz = [0, 0, 0, oBv[3] - 10, oBv[4] - 10, oBv[5] - 10, oBv[6] - 10, 0, 0, 0, 0, 0]
    ...
    if k[7] == 'Фасад' and int(visota) >= 266:
        txt_ss = open('rs/53/f.txt', 'r').read()
        RAMX, RAMY = 74, 44.5
        ...
        RAMMX = 74 if int(visota) > 319 else 74 - (320 - int(visota)) * 0.2
        RAMK = 67.54 + (int(visota) - 360) * 0.08 if int(visota) > 360 else 67.5 - (360 - int(visota)) * 0.025
        yt0 = (int(visota) - 2 * RAMK) / 4
        xt0 = (RAMMX - RAMY) / 2
        rRr = (xt0 * xt0 + yt0 * yt0) / (2 * xt0)

        for i in range(11, -1, -1):
            xo[i] = ((rRr - xt0) / rRr) * oBv[i]
            yo[i] = (yt0 / rRr) * oBv[i]
            if RAMK <= (RAMY + oBv[i]):
                RAM[i] = 0.001
                g[i] = rRr + oBv[i] - ((rRr + oBv[i]) ** 2 - (RAMY + oBv[i] - RAMK) ** 2) ** 0.5
            else:
                RAM[i] = RAMK - RAMY - oBv[i]
                g[i] = 0

            txt_ss = txt_ss.replace("g" + str(i), str(round(g[i], 3)))
            txt_ss = txt_ss.replace("xo" + str(i), str(round(xo[i], 3)))
            txt_ss = txt_ss.replace("yo" + str(i), str(round(yo[i], 3)))
            txt_ss = txt_ss.replace("RAM" + str(i), str(round(RAM[i], 3)))

        txt_ss = txt_ss.replace("xt0", f'{xt0:.3f}')
        txt_ss = txt_ss.replace("yt0", f'{yt0:.3f}')
        txt_ss = txt_ss.replace("rRr", f'{rRr:.3f}')
        txt_ss = txt_ss.replace("RAMMX", str(RAMMX))
        txt_ss = txt_ss.replace("RAMK", f'{RAMK:.3f}')

    elif k[7] == 'СТЕКЛО' and int(visota) >= 266:
        oBv[1] = oBv[8]
        txt_ss = open('rs/53/st.txt', 'r').read()
        RAMX, RAMY = 74, 44.5
        BRAMX = 74 - (320 - int(visota)) * 0.2 if int(visota) <= 320 else 74
        ...
        txt_ss = open('rs/53/xl.txt', 'r').read()
        RAMX, RAMY = 74, 44.5
        ...
        oBv[1] = oBv[8]
        txt_ss = open('rs/53/xlst.txt', 'r').read()
        RAMX, RAMY = 74, 44.5
        BRAMX = 74 - (320 - int(dlina)) * 0.2 if int(dlina) <= 320 else 74
        ...
        txt_ss = open('rs/53/but.txt', 'r').read()
        oBv[1] = oBv[0]
        RAMX, RAMY = 74, ram[1]
        if int(visota) <= 270:
        ...
        RAMMX, RAMK = 71, 44.5
        yt0 = (int(visota) - 2 * RAMY) / 4
        xt0 = (RAMMX - RAMK) / 2
        rRr = (xt0 ** 2 + yt0 ** 2) / (2 * xt0)
        ...
        txt_ss = open('rs/53/butsl.txt', 'r').read()
        if 140 <= int(visota) < 155:
            txt_ss = open('rs/53/butsl2.txt', 'r').read()
        RAMX = 66 if 153 < int(visota) < 190 else 51
        RAMY, oBv[0] = 29.5, 17
        ...
        x1 = 44.5 + rRr2
        y1 = ((rRr1 + rRr2) ** 2 - (x1 + rRr1 - RAMX) ** 2) ** 0.5 + RAMY
        rRr3 = (int(visota) / 2 - y1) / sin22 - rRr2
        x2 = x1 - (((int(visota) / 2 - y1) / sin22) ** 2 - (int(visota) / 2 - y1) ** 2) ** 0.5
        y2 = int(visota) / 2
        x3 = x2 + (x1 - x2) / (rRr2 + rRr3) * rRr3
        y3 = y2 - (y2 - y1) / (rRr2 + rRr3) * rRr3
        ty0 = (y1 - RAMY) / (rRr1 + rRr2) * rRr1
        tx0 = rRr1 - (x1 + rRr1 - RAMX) * rRr1 / (rRr1 + rRr2)

        for i in range(10):
            ux[i] = ((rRr1 - tx0) / rRr1) * oBv[i]
            uy[i] = (ty0 / rRr1) * oBv[i]
            h[i] = RAMX - rRr1 + ((rRr1 + oBv[i]) ** 2 - oBv[i] ** 2) ** 0.5
            zy[i] = (y1 - y3) * (rRr2 - oBv[i]) / rRr2
            zx[i] = (x1 - x3) * (rRr2 - oBv[i]) / rRr2
            txt_ss = txt_ss.replace("h" + str(i), f'{h[i]:.3f}')
            txt_ss = txt_ss.replace("ux" + str(i), f'{ux[i]:.3f}')
            txt_ss = txt_ss.replace("uy" + str(i), f'{uy[i]:.3f}')
            txt_ss = txt_ss.replace("zy" + str(i), f'{zy[i]:.3f}')
        ...
        RX, RY, RAMK = 71, 29.5, 44.5
        ty1 = (int(visota) - 2 * RAMY) / 4
        tx1 = (RX - RAMK) / 2
        rRr = (tx1 ** 2 + ty1 ** 2) / (2 * tx1)

        for i in range(11, -1, -1):
            xo[i] = ((rRr - tx1) / rRr) * oBv[i]
            yo[i] = (ty1 / rRr) * oBv[i]
            g[i] = rRr + oBv[i] - ((rRr + oBv[i]) ** 2 - oBv[i] ** 2) ** 0.5
            n[i] = rRr + oBz[i] - ((rRr + oBz[i]) ** 2 - oBz[i] ** 2) ** 0.5
        ...
        txt_ss = txt_ss.replace("rRr2", f'{rRr2:.3f}')
        txt_ss = txt_ss.replace("rRr3", f'{rRr3:.3f}')
        txt_ss = txt_ss.replace("x1", f'{x1:.3f}')
        txt_ss = txt_ss.replace("x2", f'{x2:.3f}')
        txt_ss = txt_ss.replace("x3", f'{x3:.3f}')
        txt_ss = txt_ss.replace("y1", f'{y1:.3f}')
        txt_ss = txt_ss.replace("y2", f'{y2:.3f}')
        txt_ss = txt_ss.replace("y3", f'{y3:.3f}')
        txt_ss = txt_ss.replace("RX", str(RX))
        txt_ss = txt_ss.replace("RY", str(RY))
        txt_ss = txt_ss.replace("RAMK", f'{RAMK:.3f}')
        txt_ss = txt_ss.replace("RAMX", str(RAMX))
        txt_ss = txt_ss.replace("RAMY", str(RAMY))
        txt_ss = txt_ss.replace("Kant", str(Kant))
