def nc_113(k, dlina, visota, RAM):
    txt_ss = ''
    konec, shag, gl = 0, 38.0, 12.501

    oBv = [29.5, 29, 12.2, 22.3001, 20.4301, 13.6001, 11.7301, 27.5, 22, 19]

    if k[7] in ('Фасад', 'Хлеб'):
        txt_ss = open('rs/113/f.txt', 'r').read()
        ...
        txt_ss = open('rs/113/ps.txt', 'r').read()
    ...
    if k[9] == "БПл":
        txt_ss = txt_ss.replace('Z=t-12.501', 'Z=t-9.5').replace('ZA="t-12.501"', 'ZA="t-9.5"')
